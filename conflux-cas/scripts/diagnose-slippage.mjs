...
const fmt = (wei, dec = 18) => (Number(wei) / 10 ** dec).toFixed(dec === 18 ? 6 : 4);

async function analyseJob(label, tokenIn, tokenOut, amountIn, minAmountOut, targetPrice) {
  console.log(`\n${'─'.repeat(60)}`);
  console.log(`JOB: ${label}`);
  console.log(`tokenIn  : ${tokenIn} (${await symbol(tokenIn)}, ${await decimals(tokenIn)} dec)`);
  console.log(`tokenOut : ${tokenOut} (${await symbol(tokenOut)}, ${await decimals(tokenOut)} dec)`);
  console.log(`amountIn : ${amountIn} (${fmt(amountIn)} tokens)`);
  console.log(`minOut   : ${minAmountOut} (${fmt(minAmountOut)} tokens)`);
  console.log(`targetPx : ${targetPrice} (${fmt(targetPrice)} — 1 tokenIn = this many tokenOut)`);
  ...
  if (res) {
    ...
    console.log(`Spot price (r1/r0 × 1e18): ${spotPrice} (${fmt(spotPrice)} tokenOut per tokenIn)`);
  }

  // Oracle quote (1 unit of tokenIn)
  const oracle = await getAmountsOut(10n ** 18n, [tokenIn, tokenOut]).catch(() => null);
  console.log(`\nOracle getAmountsOut(1e18): ${oracle ? oracle[1] + ' = ' + fmt(oracle[1]) + ' tokenOut' : 'FAILED'}`);
  console.log(`Oracle >= targetPrice?     ${oracle ? (oracle[1] >= BigInt(targetPrice) ? '✅ YES (price condition would pass)' : '❌ NO (price condition would fail)') : 'N/A'}`);

  // Actual swap quote for this specific amountIn
  const quote = await getAmountsOut(BigInt(amountIn), [tokenIn, tokenOut]).catch(() => null);
  if (quote) {
    const amtOut = quote[1];
    console.log(`\nSwap quote getAmountsOut(${amountIn}): ${amtOut} = ${fmt(amtOut)} tokenOut`);
    console.log(`minAmountOut required              : ${minAmountOut} = ${fmt(minAmountOut)} tokenOut`);
    const diff = Number(amtOut - BigInt(minAmountOut)) / Number(BigInt(minAmountOut)) * 100;
    console.log(`Difference                         : ${diff.toFixed(2)}%`);
    console.log(`Swap would PASS slippage check?    : ${amtOut >= BigInt(minAmountOut) ? '✅ YES' : '❌ NO — Slippage exceeded'}`);
  } else {
